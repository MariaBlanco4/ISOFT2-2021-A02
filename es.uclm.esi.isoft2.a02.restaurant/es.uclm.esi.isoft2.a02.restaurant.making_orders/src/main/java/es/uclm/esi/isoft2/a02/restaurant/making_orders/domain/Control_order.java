	...
	public static int checkOrder(int n_table, String date, Turn turn, 
		...
		if(ot != null) {
			...
			if(state == State.Asking) {
				...
				for(int i = 0 ; i < dishes.length && order != -1 && order != -2 && order != -3 || dishes[i+1]!=null; i++) {
					ds = Dish.readDish(dishes[i]);
					if(ds != null) {
						current_ingredients = ds.getIngredients(); //Array of each ingredient
						for(int j = 0; j<current_ingredients.length && order != -1 && order != -2 && order != -3 && dishes!=null; j++) {
				...
				for(int i = 0 ; i<drinks.length && order !=-1 && order != -3 && order != -2 && drinks!=null; i++) {
