...
public class Control_operational_table {
	public static boolean changeState(State new_state, int n_table, String date, Turn turn) throws Exception{
		...
		if (opt==null) {
			...
			if (new_state == State.Asking || new_state == State.Waiting_for_food || new_state == State.Served || new_state == State.Waiting ||
					new_state == State.Paying || new_state == State.In_preparation || new_state == State.Free) {
			...
			double differenceTime = (Double.parseDouble(parts_actTime[0])*3600 + Double.parseDouble(parts_actTime[1]) * 60 + Double.parseDouble(parts_actTime[2])) 
					- (Double.parseDouble(parts_lastTC[0])*3600 + Double.parseDouble(parts_lastTC[1]) * 60 + Double.parseDouble(parts_lastTC[2]));
